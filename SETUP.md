...
4. Run the migrations in order (numbered files in `sql/migrations/`):
   ...
   mysql -u <user> -p scrappydolls < sql/migrations/011_promo_bar.sql
   ```
   ...
   `site_settings` table behind the offer bar (see **Operational notes**), and 011 renames its
   keys after the bar moved from a corner banner to a full-width strip.
...
**Offer bar** card at the top, write the offer (as long as you like — it wraps), put the code
in the code box, pick a color, tick **Show it on the site**, Save. A colored strip runs across
the top of every public page — home, shop, doll pages, cart, contact, legal — until it's
...
| `admin/bulk-edit.php` | Apply inline-edit changes from `/admin/products.php?mode=edit` |
